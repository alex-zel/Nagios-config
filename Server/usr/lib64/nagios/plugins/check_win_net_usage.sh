...
if [[ ${detect} -eq 1 ]]; then
  if [[ -n ${password} ]]; then
    ${pluginlocation}/check_nt -t 30 -H ${host} -p ${insertport} -s ${password} -v INSTANCES -l "Network Interface" | sed "s/OK&//"; exit 0
  else
    ${pluginlocation}/check_nt -t 30 -H ${host} -p ${insertport} -v INSTANCES -l "Network Interface" | sed "s/OK&//"; exit 0
  fi
fi
...
if [[ -n ${password} ]]
then
bytes_in=$(${pluginlocation}/check_nt -t 30 -H ${host} -p ${insertport} -s ${password} -v COUNTER -l "\Network Interface(${interface})\Bytes Received/sec")
bytes_out=$(${pluginlocation}/check_nt -t 30 -H ${host} -p ${insertport} -s ${password} -v COUNTER -l "\Network Interface(${interface})\Bytes Sent/sec")
else
# Without password
bytes_in=$(${pluginlocation}/check_nt -t 30 -H ${host} -p ${insertport} -v COUNTER -l "\Network Interface(${interface})\Bytes Received/sec")
bytes_out=$(${pluginlocation}/check_nt -t 30 -H ${host} -p ${insertport} -v COUNTER -l "\Network Interface(${interface})\Bytes Sent/sec")
fi
